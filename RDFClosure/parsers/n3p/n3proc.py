...
class N3Processor(n3p.N3Parser):
   ...
   def onToken(self, prod, tok):
      if self.productions:
         ...
         # This is to handle the true/false...
         if parentProd == "symbol" and (tok == "true" or tok == "false") :
             if tok == "true" :
                 lit = Literal("true",datatype=XSD.boolean)
             elif tok == "false" :
                 lit = Literal("false",datatype=XSD.boolean)
             if self.paths:
   ...
   def nodeToken(self, prod, tok):
      ...
      def numericliteral(prod, tok):
         if '.' in tok:
            tok = str(float(tok))
            lit = Literal(tok, datatype=XSD.decimal)
         else:
            tok = str(int(tok))
