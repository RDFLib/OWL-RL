...
class Options:
	def __init__(self,form) :
		...
		if "source_1" 		in form.keys() 	: 	self.sources.append(form["source_1"].value)
		if "source_2" 		in form.keys() 	: 	self.sources.append(form["source_2"].value)
		
		if "text" 			in form.keys() 	: 	self.text = form["text"].value
		if "format" 		in form.keys()	:	self.format = form["format"].value
		if "iformat" 		in form.keys()	:	
			v = form["iformat"].value
			if v == "xml" or v == "turtle" : self.iformat = v	
			
		if "fullClosure" in form.keys() and form["fullClosure"].value == "yes" :
			self.owlClosure  = "yes"
			...
			if "owlClosure" 	in form.keys()	:	self.owlClosure = form["owlClosure"].value
			if "rdfsClosure"	in form.keys()	:	self.rdfsClosure = form["rdfsClosure"].value
			if "owlExtras" 		in form.keys() 	:	self.owlExtras = form["owlExtras"].value
			if "axioms" 		in form.keys() 	:	self.axioms = form["axioms"].value
			if "daxioms" 		in form.keys() 	:	self.daxioms = form["daxioms"].value

		# this one is for backward compatibility...
		if "uri" 			in form.keys() 	: 	self.sources.append(form["uri"].value)		
		if "source" 		in form.keys() 	: 	self.sources.append(form["source"].value)
